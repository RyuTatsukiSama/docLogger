...
REM Name of the files
set "counter_file=counter.txt"

REM Check if file exist if not create with 0
if not exist "%counter_file%" (
    echo 0 > "%counter_file%"
)

REM Read current and increment
set /p nb=<"%counter_file%"
set /a nb+=1

REM Write new number
echo %nb% > "%counter_file%"

echo This script has been launched %nb% times
